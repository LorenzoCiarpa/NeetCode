# NeetCode Solutions

Repository delle mie soluzioni sincronizzate automaticamente da [NeetCode.io](https://neetcode.io)
tramite la funzione **GitHub Sync**. Il tracker qui sotto viene rigenerato a ogni push
da una GitHub Action, quindi resta sempre allineato alle submission.

## Struttura

Le soluzioni sono organizzate come le scrive NeetCode:

```
<topic-folder>/
  <problem-slug>/
    submission-0.py   ← primo tentativo
    submission-1.py   ← tentativo successivo
```

I file multipli per problema sono voluti: mostrano l'iterazione nel tempo, non solo
la versione finale.

## Progresso

<!-- PROGRESS:START -->
### NeetCode 250 &nbsp;·&nbsp; 101 / 250 &nbsp;·&nbsp; 40%

`██████████░░░░░░░░░░░░░░`

| | 🟢 Easy | 🟡 Medium | 🔴 Hard |
|:--|:--:|:--:|:--:|
| **Risolti** | **38** | **62** | **1** |
| **Totale lista** | 60 | 155 | 35 |

| Pattern | Risolti | Totale | Progresso |
|---------|--------:|-------:|:----------|
| Arrays & Hashing | 12 | 22 | `███████░░░░░` 55% |
| Two Pointers | 6 | 13 | `██████░░░░░░` 46% |
| Sliding Window | 4 | 9 | `█████░░░░░░░` 44% |
| Stack | 7 | 15 | `██████░░░░░░` 47% |
| Binary Search | 5 | 14 | `████░░░░░░░░` 36% |
| Linked List | 6 | 14 | `█████░░░░░░░` 43% |
| Trees | 16 | 23 | `████████░░░░` 70% |
| Tries | 2 | 4 | `██████░░░░░░` 50% |
| Heap / Priority Queue | 5 | 12 | `█████░░░░░░░` 42% |
| Backtracking | 8 | 16 | `██████░░░░░░` 50% |
| Graphs | 10 | 21 | `██████░░░░░░` 48% |
| Advanced Graphs | 2 | 10 | `██░░░░░░░░░░` 20% |
| 1-D Dynamic Programming | 4 | 17 | `███░░░░░░░░░` 24% |
| 2-D Dynamic Programming | 2 | 16 | `██░░░░░░░░░░` 12% |
| Greedy | 5 | 14 | `████░░░░░░░░` 36% |
| Intervals | 0 | 7 | `░░░░░░░░░░░░` 0% |
| Math & Geometry | 2 | 13 | `██░░░░░░░░░░` 15% |
| Bit Manipulation | 5 | 10 | `██████░░░░░░` 50% |

### Dettaglio per pattern

<details>
<summary><b>Arrays & Hashing</b> &nbsp;·&nbsp; 12/22 (55%)</summary>

- [ ] 🟡 [Best Time to Buy And Sell Stock II](https://neetcode.io/problems/best-time-to-buy-and-sell-stock-ii)
- [x] 🟢 [Concatenation of Array](https://neetcode.io/problems/concatenation-of-array)
- [x] 🟢 [Contains Duplicate](https://neetcode.io/problems/duplicate-integer)
- [ ] 🟢 [Design HashMap](https://neetcode.io/problems/design-hashmap)
- [ ] 🟢 [Design HashSet](https://neetcode.io/problems/design-hashset)
- [x] 🟡 [Encode and Decode Strings](https://neetcode.io/problems/string-encode-and-decode)
- [ ] 🔴 [First Missing Positive](https://neetcode.io/problems/first-missing-positive)
- [x] 🟡 [Group Anagrams](https://neetcode.io/problems/anagram-groups)
- [ ] 🟢 [Longest Common Prefix](https://neetcode.io/problems/longest-common-prefix)
- [x] 🟡 [Longest Consecutive Sequence](https://neetcode.io/problems/longest-consecutive-sequence)
- [ ] 🟢 [Majority Element](https://neetcode.io/problems/majority-element)
- [ ] 🟡 [Majority Element II](https://neetcode.io/problems/majority-element-ii)
- [x] 🟡 [Product of Array Except Self](https://neetcode.io/problems/products-of-array-discluding-self)
- [x] 🟡 [Range Sum Query 2D Immutable](https://neetcode.io/problems/range-sum-query-2d-immutable)
- [x] 🟢 [Remove Element](https://neetcode.io/problems/remove-element)
- [ ] 🟡 [Sort Colors](https://neetcode.io/problems/sort-colors)
- [ ] 🟡 [Sort an Array](https://neetcode.io/problems/sort-an-array)
- [ ] 🟡 [Subarray Sum Equals K](https://neetcode.io/problems/subarray-sum-equals-k)
- [x] 🟡 [Top K Frequent Elements](https://neetcode.io/problems/top-k-elements-in-list)
- [x] 🟢 [Two Sum](https://neetcode.io/problems/two-integer-sum)
- [x] 🟢 [Valid Anagram](https://neetcode.io/problems/is-anagram)
- [x] 🟡 [Valid Sudoku](https://neetcode.io/problems/valid-sudoku)

</details>

<details>
<summary><b>Two Pointers</b> &nbsp;·&nbsp; 6/13 (46%)</summary>

- [x] 🟡 [3Sum](https://neetcode.io/problems/three-integer-sum)
- [ ] 🟡 [4Sum](https://neetcode.io/problems/4sum)
- [ ] 🟡 [Boats to Save People](https://neetcode.io/problems/boats-to-save-people)
- [x] 🟡 [Container With Most Water](https://neetcode.io/problems/max-water-container)
- [x] 🟢 [Merge Sorted Array](https://neetcode.io/problems/merge-sorted-array)
- [ ] 🟢 [Merge Strings Alternately](https://neetcode.io/problems/merge-strings-alternately)
- [x] 🟢 [Remove Duplicates From Sorted Array](https://neetcode.io/problems/remove-duplicates-from-sorted-array)
- [ ] 🟢 [Reverse String](https://neetcode.io/problems/reverse-string)
- [ ] 🟡 [Rotate Array](https://neetcode.io/problems/rotate-array)
- [ ] 🔴 [Trapping Rain Water](https://neetcode.io/problems/trapping-rain-water)
- [x] 🟡 [Two Sum II Input Array Is Sorted](https://neetcode.io/problems/two-integer-sum-ii)
- [x] 🟢 [Valid Palindrome](https://neetcode.io/problems/is-palindrome)
- [ ] 🟢 [Valid Palindrome II](https://neetcode.io/problems/valid-palindrome-ii)

</details>

<details>
<summary><b>Sliding Window</b> &nbsp;·&nbsp; 4/9 (44%)</summary>

- [x] 🟢 [Best Time to Buy And Sell Stock](https://neetcode.io/problems/buy-and-sell-crypto)
- [x] 🟢 [Contains Duplicate II](https://neetcode.io/problems/contains-duplicate-ii)
- [ ] 🟡 [Find K Closest Elements](https://neetcode.io/problems/find-k-closest-elements)
- [ ] 🟡 [Longest Repeating Character Replacement](https://neetcode.io/problems/longest-repeating-substring-with-replacement)
- [x] 🟡 [Longest Substring Without Repeating Characters](https://neetcode.io/problems/longest-substring-without-duplicates)
- [x] 🟡 [Minimum Size Subarray Sum](https://neetcode.io/problems/minimum-size-subarray-sum)
- [ ] 🔴 [Minimum Window Substring](https://neetcode.io/problems/minimum-window-with-characters)
- [ ] 🟡 [Permutation In String](https://neetcode.io/problems/permutation-string)
- [ ] 🔴 [Sliding Window Maximum](https://neetcode.io/problems/sliding-window-maximum)

</details>

<details>
<summary><b>Stack</b> &nbsp;·&nbsp; 7/15 (47%)</summary>

- [ ] 🟡 [Asteroid Collision](https://neetcode.io/problems/asteroid-collision)
- [x] 🟢 [Baseball Game](https://neetcode.io/problems/baseball-game)
- [x] 🟡 [Car Fleet](https://neetcode.io/problems/car-fleet)
- [x] 🟡 [Daily Temperatures](https://neetcode.io/problems/daily-temperatures)
- [ ] 🟡 [Decode String](https://neetcode.io/problems/decode-string)
- [x] 🟡 [Evaluate Reverse Polish Notation](https://neetcode.io/problems/evaluate-reverse-polish-notation)
- [ ] 🟡 [Generate Parentheses](https://neetcode.io/problems/generate-parentheses)
- [ ] 🟢 [Implement Queue using Stacks](https://neetcode.io/problems/implement-queue-using-stacks)
- [x] 🟢 [Implement Stack Using Queues](https://neetcode.io/problems/implement-stack-using-queues)
- [ ] 🔴 [Largest Rectangle In Histogram](https://neetcode.io/problems/largest-rectangle-in-histogram)
- [ ] 🔴 [Maximum Frequency Stack](https://neetcode.io/problems/maximum-frequency-stack)
- [x] 🟡 [Min Stack](https://neetcode.io/problems/minimum-stack)
- [ ] 🟡 [Online Stock Span](https://neetcode.io/problems/online-stock-span)
- [ ] 🟡 [Simplify Path](https://neetcode.io/problems/simplify-path)
- [x] 🟢 [Valid Parentheses](https://neetcode.io/problems/validate-parentheses)

</details>

<details>
<summary><b>Binary Search</b> &nbsp;·&nbsp; 5/14 (36%)</summary>

- [x] 🟢 [Binary Search](https://neetcode.io/problems/binary-search)
- [ ] 🟡 [Capacity to Ship Packages Within D Days](https://neetcode.io/problems/capacity-to-ship-packages-within-d-days)
- [x] 🟡 [Find Minimum In Rotated Sorted Array](https://neetcode.io/problems/find-minimum-in-rotated-sorted-array)
- [ ] 🔴 [Find in Mountain Array](https://neetcode.io/problems/find-in-mountain-array)
- [x] 🟢 [Guess Number Higher Or Lower](https://neetcode.io/problems/guess-number-higher-or-lower)
- [x] 🟡 [Koko Eating Bananas](https://neetcode.io/problems/eating-bananas)
- [ ] 🔴 [Median of Two Sorted Arrays](https://neetcode.io/problems/median-of-two-sorted-arrays)
- [ ] 🟡 [Search In Rotated Sorted Array](https://neetcode.io/problems/find-target-in-rotated-sorted-array)
- [ ] 🟡 [Search In Rotated Sorted Array II](https://neetcode.io/problems/search-in-rotated-sorted-array-ii)
- [ ] 🟢 [Search Insert Position](https://neetcode.io/problems/search-insert-position)
- [x] 🟡 [Search a 2D Matrix](https://neetcode.io/problems/search-2d-matrix)
- [ ] 🔴 [Split Array Largest Sum](https://neetcode.io/problems/split-array-largest-sum)
- [ ] 🟢 [Sqrt(x)](https://neetcode.io/problems/sqrtx)
- [ ] 🟡 [Time Based Key Value Store](https://neetcode.io/problems/time-based-key-value-store)

</details>

<details>
<summary><b>Linked List</b> &nbsp;·&nbsp; 6/14 (43%)</summary>

- [x] 🟡 [Add Two Numbers](https://neetcode.io/problems/add-two-numbers)
- [ ] 🟡 [Copy List With Random Pointer](https://neetcode.io/problems/copy-linked-list-with-random-pointer)
- [ ] 🟡 [Design Circular Queue](https://neetcode.io/problems/design-circular-queue)
- [x] 🟡 [Find The Duplicate Number](https://neetcode.io/problems/find-duplicate-integer)
- [ ] 🔴 [LFU Cache](https://neetcode.io/problems/lfu-cache)
- [ ] 🟡 [LRU Cache](https://neetcode.io/problems/lru-cache)
- [x] 🟢 [Linked List Cycle](https://neetcode.io/problems/linked-list-cycle-detection)
- [ ] 🔴 [Merge K Sorted Lists](https://neetcode.io/problems/merge-k-sorted-linked-lists)
- [x] 🟢 [Merge Two Sorted Lists](https://neetcode.io/problems/merge-two-sorted-linked-lists)
- [x] 🟡 [Remove Nth Node From End of List](https://neetcode.io/problems/remove-node-from-end-of-linked-list)
- [ ] 🟡 [Reorder List](https://neetcode.io/problems/reorder-linked-list)
- [x] 🟢 [Reverse Linked List](https://neetcode.io/problems/reverse-a-linked-list)
- [ ] 🟡 [Reverse Linked List II](https://neetcode.io/problems/reverse-linked-list-ii)
- [ ] 🔴 [Reverse Nodes In K Group](https://neetcode.io/problems/reverse-nodes-in-k-group)

</details>

<details>
<summary><b>Trees</b> &nbsp;·&nbsp; 16/23 (70%)</summary>

- [x] 🟢 [Balanced Binary Tree](https://neetcode.io/problems/balanced-binary-tree)
- [x] 🟢 [Binary Tree Inorder Traversal](https://neetcode.io/problems/binary-tree-inorder-traversal)
- [ ] 🟡 [Binary Tree Level Order Traversal](https://neetcode.io/problems/level-order-traversal-of-binary-tree)
- [ ] 🔴 [Binary Tree Maximum Path Sum](https://neetcode.io/problems/binary-tree-maximum-path-sum)
- [x] 🟢 [Binary Tree Postorder Traversal](https://neetcode.io/problems/binary-tree-postorder-traversal)
- [x] 🟢 [Binary Tree Preorder Traversal](https://neetcode.io/problems/binary-tree-preorder-traversal)
- [x] 🟡 [Binary Tree Right Side View](https://neetcode.io/problems/binary-tree-right-side-view)
- [ ] 🟡 [Construct Binary Tree From Preorder And Inorder Traversal](https://neetcode.io/problems/binary-tree-from-preorder-and-inorder-traversal)
- [ ] 🟡 [Construct Quad Tree](https://neetcode.io/problems/construct-quad-tree)
- [x] 🟡 [Count Good Nodes In Binary Tree](https://neetcode.io/problems/count-good-nodes-in-binary-tree)
- [ ] 🟡 [Delete Leaves With a Given Value](https://neetcode.io/problems/delete-leaves-with-a-given-value)
- [x] 🟡 [Delete Node in a BST](https://neetcode.io/problems/delete-node-in-a-bst)
- [x] 🟢 [Diameter of Binary Tree](https://neetcode.io/problems/binary-tree-diameter)
- [ ] 🟡 [House Robber III](https://neetcode.io/problems/house-robber-iii)
- [x] 🟡 [Insert into a Binary Search Tree](https://neetcode.io/problems/insert-into-a-binary-search-tree)
- [x] 🟢 [Invert Binary Tree](https://neetcode.io/problems/invert-a-binary-tree)
- [x] 🟡 [Kth Smallest Element In a Bst](https://neetcode.io/problems/kth-smallest-integer-in-bst)
- [x] 🟡 [Lowest Common Ancestor of a Binary Search Tree](https://neetcode.io/problems/lowest-common-ancestor-in-binary-search-tree)
- [x] 🟢 [Maximum Depth of Binary Tree](https://neetcode.io/problems/depth-of-binary-tree)
- [x] 🟢 [Same Tree](https://neetcode.io/problems/same-binary-tree)
- [ ] 🔴 [Serialize And Deserialize Binary Tree](https://neetcode.io/problems/serialize-and-deserialize-binary-tree)
- [x] 🟢 [Subtree of Another Tree](https://neetcode.io/problems/subtree-of-a-binary-tree)
- [x] 🟡 [Validate Binary Search Tree](https://neetcode.io/problems/valid-binary-search-tree)

</details>

<details>
<summary><b>Tries</b> &nbsp;·&nbsp; 2/4 (50%)</summary>

- [x] 🟡 [Design Add And Search Words Data Structure](https://neetcode.io/problems/design-word-search-data-structure)
- [ ] 🟡 [Extra Characters in a String](https://neetcode.io/problems/extra-characters-in-a-string)
- [x] 🟡 [Implement Trie Prefix Tree](https://neetcode.io/problems/implement-prefix-tree)
- [ ] 🔴 [Word Search II](https://neetcode.io/problems/search-for-word-ii)

</details>

<details>
<summary><b>Heap / Priority Queue</b> &nbsp;·&nbsp; 5/12 (42%)</summary>

- [ ] 🟡 [Car Pooling](https://neetcode.io/problems/car-pooling)
- [ ] 🟡 [Design Twitter](https://neetcode.io/problems/design-twitter-feed)
- [x] 🔴 [Find Median From Data Stream](https://neetcode.io/problems/find-median-in-a-data-stream)
- [ ] 🔴 [IPO](https://neetcode.io/problems/ipo)
- [x] 🟡 [K Closest Points to Origin](https://neetcode.io/problems/k-closest-points-to-origin)
- [x] 🟡 [Kth Largest Element In An Array](https://neetcode.io/problems/kth-largest-element-in-an-array)
- [x] 🟢 [Kth Largest Element In a Stream](https://neetcode.io/problems/kth-largest-integer-in-a-stream)
- [x] 🟢 [Last Stone Weight](https://neetcode.io/problems/last-stone-weight)
- [ ] 🟡 [Longest Happy String](https://neetcode.io/problems/longest-happy-string)
- [ ] 🟡 [Reorganize String](https://neetcode.io/problems/reorganize-string)
- [ ] 🟡 [Single Threaded CPU](https://neetcode.io/problems/single-threaded-cpu)
- [ ] 🟡 [Task Scheduler](https://neetcode.io/problems/task-scheduling)

</details>

<details>
<summary><b>Backtracking</b> &nbsp;·&nbsp; 8/16 (50%)</summary>

- [x] 🟡 [Combination Sum](https://neetcode.io/problems/combination-target-sum)
- [x] 🟡 [Combination Sum II](https://neetcode.io/problems/combination-target-sum-ii)
- [x] 🟡 [Combinations](https://neetcode.io/problems/combinations)
- [x] 🟡 [Letter Combinations of a Phone Number](https://neetcode.io/problems/combinations-of-a-phone-number)
- [ ] 🟡 [Matchsticks to Square](https://neetcode.io/problems/matchsticks-to-square)
- [ ] 🔴 [N Queens](https://neetcode.io/problems/n-queens)
- [ ] 🔴 [N Queens II](https://neetcode.io/problems/n-queens-ii)
- [ ] 🟡 [Palindrome Partitioning](https://neetcode.io/problems/palindrome-partitioning)
- [ ] 🟡 [Partition to K Equal Sum Subsets](https://neetcode.io/problems/partition-to-k-equal-sum-subsets)
- [x] 🟡 [Permutations](https://neetcode.io/problems/permutations)
- [ ] 🟡 [Permutations II](https://neetcode.io/problems/permutations-ii)
- [x] 🟡 [Subsets](https://neetcode.io/problems/subsets)
- [x] 🟡 [Subsets II](https://neetcode.io/problems/subsets-ii)
- [ ] 🟢 [Sum of All Subsets XOR Total](https://neetcode.io/problems/sum-of-all-subset-xor-totals)
- [ ] 🔴 [Word Break II](https://neetcode.io/problems/word-break-ii)
- [x] 🟡 [Word Search](https://neetcode.io/problems/search-for-word)

</details>

<details>
<summary><b>Graphs</b> &nbsp;·&nbsp; 10/21 (48%)</summary>

- [ ] 🟡 [Accounts Merge](https://neetcode.io/problems/accounts-merge)
- [x] 🟡 [Clone Graph](https://neetcode.io/problems/clone-graph)
- [x] 🟡 [Course Schedule](https://neetcode.io/problems/course-schedule)
- [x] 🟡 [Course Schedule II](https://neetcode.io/problems/course-schedule-ii)
- [x] 🟡 [Course Schedule IV](https://neetcode.io/problems/course-schedule-iv)
- [ ] 🟡 [Evaluate Division](https://neetcode.io/problems/evaluate-division)
- [ ] 🟢 [Find the Town Judge](https://neetcode.io/problems/find-the-town-judge)
- [x] 🟡 [Graph Valid Tree](https://neetcode.io/problems/valid-tree)
- [ ] 🟢 [Island Perimeter](https://neetcode.io/problems/island-perimeter)
- [x] 🟡 [Max Area of Island](https://neetcode.io/problems/max-area-of-island)
- [ ] 🟡 [Minimum Height Trees](https://neetcode.io/problems/minimum-height-trees)
- [x] 🟡 [Number of Connected Components In An Undirected Graph](https://neetcode.io/problems/count-connected-components)
- [x] 🟡 [Number of Islands](https://neetcode.io/problems/count-number-of-islands)
- [ ] 🟡 [Open The Lock](https://neetcode.io/problems/open-the-lock)
- [ ] 🟡 [Pacific Atlantic Water Flow](https://neetcode.io/problems/pacific-atlantic-water-flow)
- [x] 🟡 [Redundant Connection](https://neetcode.io/problems/redundant-connection)
- [x] 🟡 [Rotting Oranges](https://neetcode.io/problems/rotting-fruit)
- [ ] 🟡 [Surrounded Regions](https://neetcode.io/problems/surrounded-regions)
- [ ] 🟢 [Verifying An Alien Dictionary](https://neetcode.io/problems/verifying-an-alien-dictionary)
- [ ] 🟡 [Walls And Gates](https://neetcode.io/problems/islands-and-treasure)
- [ ] 🔴 [Word Ladder](https://neetcode.io/problems/word-ladder)

</details>

<details>
<summary><b>Advanced Graphs</b> &nbsp;·&nbsp; 2/10 (20%)</summary>

- [ ] 🔴 [Alien Dictionary](https://neetcode.io/problems/foreign-dictionary)
- [ ] 🔴 [Build a Matrix With Conditions](https://neetcode.io/problems/build-a-matrix-with-conditions)
- [ ] 🟡 [Cheapest Flights Within K Stops](https://neetcode.io/problems/cheapest-flight-path)
- [ ] 🔴 [Find Critical and Pseudo Critical Edges in Minimum Spanning Tree](https://neetcode.io/problems/find-critical-and-pseudo-critical-edges-in-minimum-spanning-tree)
- [ ] 🔴 [Greatest Common Divisor Traversal](https://neetcode.io/problems/greatest-common-divisor-traversal)
- [x] 🟡 [Min Cost to Connect All Points](https://neetcode.io/problems/min-cost-to-connect-points)
- [x] 🟡 [Network Delay Time](https://neetcode.io/problems/network-delay-time)
- [ ] 🟡 [Path with Minimum Effort](https://neetcode.io/problems/path-with-minimum-effort)
- [ ] 🔴 [Reconstruct Itinerary](https://neetcode.io/problems/reconstruct-flight-path)
- [ ] 🔴 [Swim In Rising Water](https://neetcode.io/problems/swim-in-rising-water)

</details>

<details>
<summary><b>1-D Dynamic Programming</b> &nbsp;·&nbsp; 4/17 (24%)</summary>

- [x] 🟢 [Climbing Stairs](https://neetcode.io/problems/climbing-stairs)
- [ ] 🟡 [Coin Change](https://neetcode.io/problems/coin-change)
- [ ] 🟡 [Combination Sum IV](https://neetcode.io/problems/combination-sum-iv)
- [ ] 🟡 [Decode Ways](https://neetcode.io/problems/decode-ways)
- [x] 🟡 [House Robber](https://neetcode.io/problems/house-robber)
- [ ] 🟡 [House Robber II](https://neetcode.io/problems/house-robber-ii)
- [ ] 🟡 [Integer Break](https://neetcode.io/problems/integer-break)
- [ ] 🟡 [Longest Increasing Subsequence](https://neetcode.io/problems/longest-increasing-subsequence)
- [x] 🟡 [Longest Palindromic Substring](https://neetcode.io/problems/longest-palindromic-substring)
- [ ] 🟡 [Maximum Product Subarray](https://neetcode.io/problems/maximum-product-subarray)
- [x] 🟢 [Min Cost Climbing Stairs](https://neetcode.io/problems/min-cost-climbing-stairs)
- [ ] 🟢 [N-th Tribonacci Number](https://neetcode.io/problems/n-th-tribonacci-number)
- [ ] 🟡 [Palindromic Substrings](https://neetcode.io/problems/palindromic-substrings)
- [ ] 🟡 [Partition Equal Subset Sum](https://neetcode.io/problems/partition-equal-subset-sum)
- [ ] 🟡 [Perfect Squares](https://neetcode.io/problems/perfect-squares)
- [ ] 🔴 [Stone Game III](https://neetcode.io/problems/stone-game-iii)
- [ ] 🟡 [Word Break](https://neetcode.io/problems/word-break)

</details>

<details>
<summary><b>2-D Dynamic Programming</b> &nbsp;·&nbsp; 2/16 (12%)</summary>

- [ ] 🟡 [Best Time to Buy And Sell Stock With Cooldown](https://neetcode.io/problems/buy-and-sell-crypto-with-cooldown)
- [ ] 🔴 [Burst Balloons](https://neetcode.io/problems/burst-balloons)
- [ ] 🟡 [Coin Change II](https://neetcode.io/problems/coin-change-ii)
- [ ] 🔴 [Distinct Subsequences](https://neetcode.io/problems/count-subsequences)
- [ ] 🟡 [Edit Distance](https://neetcode.io/problems/edit-distance)
- [ ] 🟡 [Interleaving String](https://neetcode.io/problems/interleaving-string)
- [ ] 🟡 [Last Stone Weight II](https://neetcode.io/problems/last-stone-weight-ii)
- [ ] 🟡 [Longest Common Subsequence](https://neetcode.io/problems/longest-common-subsequence)
- [ ] 🔴 [Longest Increasing Path In a Matrix](https://neetcode.io/problems/longest-increasing-path-in-matrix)
- [ ] 🟡 [Minimum Path Sum](https://neetcode.io/problems/minimum-path-sum)
- [ ] 🔴 [Regular Expression Matching](https://neetcode.io/problems/regular-expression-matching)
- [ ] 🟡 [Stone Game](https://neetcode.io/problems/stone-game)
- [ ] 🟡 [Stone Game II](https://neetcode.io/problems/stone-game-ii)
- [ ] 🟡 [Target Sum](https://neetcode.io/problems/target-sum)
- [x] 🟡 [Unique Paths](https://neetcode.io/problems/count-paths)
- [x] 🟡 [Unique Paths II](https://neetcode.io/problems/unique-paths-ii)

</details>

<details>
<summary><b>Greedy</b> &nbsp;·&nbsp; 5/14 (36%)</summary>

- [ ] 🔴 [Candy](https://neetcode.io/problems/candy)
- [ ] 🟡 [Dota2 Senate](https://neetcode.io/problems/dota2-senate)
- [x] 🟡 [Gas Station](https://neetcode.io/problems/gas-station)
- [ ] 🟡 [Hand of Straights](https://neetcode.io/problems/hand-of-straights)
- [x] 🟡 [Jump Game](https://neetcode.io/problems/jump-game)
- [x] 🟡 [Jump Game II](https://neetcode.io/problems/jump-game-ii)
- [ ] 🟡 [Jump Game VII](https://neetcode.io/problems/jump-game-vii)
- [ ] 🟢 [Lemonade Change](https://neetcode.io/problems/lemonade-change)
- [ ] 🟡 [Longest Turbulent Subarray](https://neetcode.io/problems/longest-turbulent-subarray)
- [x] 🟡 [Maximum Subarray](https://neetcode.io/problems/maximum-subarray)
- [x] 🟡 [Maximum Sum Circular Subarray](https://neetcode.io/problems/maximum-sum-circular-subarray)
- [ ] 🟡 [Merge Triplets to Form Target Triplet](https://neetcode.io/problems/merge-triplets-to-form-target)
- [ ] 🟡 [Partition Labels](https://neetcode.io/problems/partition-labels)
- [ ] 🟡 [Valid Parenthesis String](https://neetcode.io/problems/valid-parenthesis-string)

</details>

<details>
<summary><b>Intervals</b> &nbsp;·&nbsp; 0/7 (0%)</summary>

- [ ] 🟡 [Insert Interval](https://neetcode.io/problems/insert-new-interval)
- [ ] 🟢 [Meeting Rooms](https://neetcode.io/problems/meeting-schedule)
- [ ] 🟡 [Meeting Rooms II](https://neetcode.io/problems/meeting-schedule-ii)
- [ ] 🔴 [Meeting Rooms III](https://neetcode.io/problems/meeting-rooms-iii)
- [ ] 🟡 [Merge Intervals](https://neetcode.io/problems/merge-intervals)
- [ ] 🔴 [Minimum Interval to Include Each Query](https://neetcode.io/problems/minimum-interval-including-query)
- [ ] 🟡 [Non Overlapping Intervals](https://neetcode.io/problems/non-overlapping-intervals)

</details>

<details>
<summary><b>Math & Geometry</b> &nbsp;·&nbsp; 2/13 (15%)</summary>

- [ ] 🟡 [Detect Squares](https://neetcode.io/problems/count-squares)
- [ ] 🟢 [Excel Sheet Column Title](https://neetcode.io/problems/excel-sheet-column-title)
- [ ] 🟢 [Greatest Common Divisor of Strings](https://neetcode.io/problems/greatest-common-divisor-of-strings)
- [x] 🟢 [Happy Number](https://neetcode.io/problems/non-cyclical-number)
- [ ] 🟡 [Insert Greatest Common Divisors in Linked List](https://neetcode.io/problems/insert-greatest-common-divisors-in-linked-list)
- [ ] 🟡 [Multiply Strings](https://neetcode.io/problems/multiply-strings)
- [x] 🟢 [Plus One](https://neetcode.io/problems/plus-one)
- [ ] 🟡 [Pow(x, n)](https://neetcode.io/problems/pow-x-n)
- [ ] 🟢 [Roman to Integer](https://neetcode.io/problems/roman-to-integer)
- [ ] 🟡 [Rotate Image](https://neetcode.io/problems/rotate-matrix)
- [ ] 🟡 [Set Matrix Zeroes](https://neetcode.io/problems/set-zeroes-in-matrix)
- [ ] 🟡 [Spiral Matrix](https://neetcode.io/problems/spiral-matrix)
- [ ] 🟢 [Transpose Matrix](https://neetcode.io/problems/transpose-matrix)

</details>

<details>
<summary><b>Bit Manipulation</b> &nbsp;·&nbsp; 5/10 (50%)</summary>

- [ ] 🟢 [Add Binary](https://neetcode.io/problems/add-binary)
- [ ] 🟡 [Bitwise AND of Numbers Range](https://neetcode.io/problems/bitwise-and-of-numbers-range)
- [x] 🟢 [Counting Bits](https://neetcode.io/problems/counting-bits)
- [ ] 🟡 [Minimum Array End](https://neetcode.io/problems/minimum-array-end)
- [x] 🟢 [Missing Number](https://neetcode.io/problems/missing-number)
- [x] 🟢 [Number of 1 Bits](https://neetcode.io/problems/number-of-one-bits)
- [x] 🟢 [Reverse Bits](https://neetcode.io/problems/reverse-bits)
- [ ] 🟡 [Reverse Integer](https://neetcode.io/problems/reverse-integer)
- [x] 🟢 [Single Number](https://neetcode.io/problems/single-number)
- [ ] 🟡 [Sum of Two Integers](https://neetcode.io/problems/sum-of-two-integers)

</details>


### Fuori lista &nbsp;·&nbsp; 13

<details>
<summary>Risolti ma non presenti in NeetCode 250</summary>

| Problema | Difficoltà | Pattern |
|----------|:----------:|---------|
| Binary Search Tree Iterator | 🟡 Medium | Trees |
| Design Browser History | 🟡 Medium | Linked List |
| Design Linked List | 🟡 Medium | Linked List |
| Find Pivot Index | 🟢 Easy | Arrays & Hashing |
| Maximum Twin Sum Of A Linked List | 🟡 Medium | Linked List |
| Middle of the Linked List | 🟢 Easy | Linked List |
| Number of Sub Arrays of Size K and Avg Greater than or Equal to Threshold | 🟡 Medium | Sliding Window |
| Path Sum | 🟢 Easy | Trees |
| Path with Maximum Probability | 🟡 Medium | Advanced Graphs |
| Range Sum Query - Immutable | 🟢 Easy | Arrays & Hashing |
| Remove Duplicates From Sorted Array II | 🟡 Medium | Two Pointers |
| Replace Elements With Greatest Element On Right Side | 🟢 Easy | Arrays & Hashing |
| Shortest Path in Binary Matrix | 🟡 Medium | Graphs |

</details>


### Altro &nbsp;·&nbsp; 5

<details>
<summary>5 soluzioni non ancora mappate nel catalogo</summary>

| Slug | Topic | Submission |
|------|-------|-----------:|
| [`binarySearchTree`](https://neetcode.io/problems/binarySearchTree) | Data Structures & Algorithms | 1 |
| [`flood-fill`](https://neetcode.io/problems/flood-fill) | Data Structures & Algorithms | 1 |
| [`hashTable`](https://neetcode.io/problems/hashTable) | Data Structures & Algorithms | 1 |
| [`max-consecutive-ones`](https://neetcode.io/problems/max-consecutive-ones) | Data Structures & Algorithms | 2 |
| [`number-of-students-unable-to-eat-lunch`](https://neetcode.io/problems/number-of-students-unable-to-eat-lunch) | Data Structures & Algorithms | 1 |

</details>


_Linguaggi: Python_ &nbsp;·&nbsp; _Ultimo aggiornamento: 2026-09-08_
<!-- PROGRESS:END -->

## Come funziona il tracker

| File | Ruolo |
|------|-------|
| `utils/neetcode_catalog.json` | Catalogo: slug NeetCode → nome, difficoltà, pattern, liste |
| `utils/extra_problems.json` | Override e aggiunte manuali, vince sul catalogo |
| `utils/update_progress.py` | Scansiona le cartelle e riscrive il blocco `PROGRESS` |
| `.github/workflows/update-progress.yml` | Esegue lo script a ogni push |

### Cambiare lista tracciata

In cima a `utils/update_progress.py`:

```python
TRACK_LIST = "neetcode250"   # oppure "all" per il catalogo completo
```

Con `neetcode250` i problemi risolti fuori lista finiscono in una sezione
**Fuori lista**, senza sparire. Con `all` rientrano tutti nel conteggio.

### Aggiungere problemi mancanti

Se una soluzione non viene riconosciuta finisce nella sezione **Altro**. Per mapparla:

```bash
python utils/update_progress.py --report
```

Stampa gli slug sconosciuti già nel formato di `utils/extra_problems.json`: si incolla,
si correggono `name` / `difficulty` / `pattern`, e al push successivo vengono smistati
nella categoria giusta con i totali ricalcolati.

Per rigenerare in locale: `python utils/update_progress.py`

## Crediti

L'idea della dashboard di progresso nel README (contatori per difficoltà + tabella per
pattern con barre ASCII) è ripresa dal repository
[cris96spa/LeetCode](https://github.com/cris96spa/LeetCode) di
[Cristian C. Spagnuolo](https://github.com/cris96spa).

Le liste di problemi e i relativi pattern provengono da [NeetCode.io](https://neetcode.io).

## Licenza

MIT
